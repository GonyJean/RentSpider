...
moment.locale('zh-cn');
function insert(title, sum, unit, baseInfo, user, villageName, area, address, developers, postTime, url, location) {
  var info = new Info58({
    title: title,
    sum: sum,
    unit: unit,
...
function getInfo(Num) {
  superagent.get(baseUrl + "ershoufang" + "/pn" + Num) //这里设置编码
  .set("User-Agent", "Baiduspider").end(function (err, res) {
    ...
    list.each(function (i, e) {
      ...
      var sum = $(this).find(".price .sum").text().replace(/[\r\n\s+]/g, "");
      var unit = $(this).find(".price .unit").text().replace(/[\r\n\s+]/g, "");
      baseInfo.pattern = $(this).find(".baseinfo").eq(0).find("span").first().text().replace(/[\r\n\s+]/g, ""); // 格局
      baseInfo.acreage = $(this).find(".baseinfo").eq(0).find("span").eq(1).text(); // 面积
      baseInfo.orientations = $(this).find(".baseinfo").eq(0).find("span").eq(2).text(); // 朝向
      ...
      user.date = $(this).find(".jjrinfo .wlt").text().replace(/[\r\n\s+]/g, "");
      user.img = $(this).find(".jjrinfo .jjrtip img").attr("src");
      var postTime = moment().format('LL');
      var location = { lng: "", lat: "" };
      var type = '二手房';
      superagent.get(encodeURI("http://api.map.baidu.com/geocoder/v2/?address=" + "乌鲁木齐市" + area + address + "&output=XML&ak=" + baiduAK + "&callback=showLocation")).end(function (err, res) {
        if (err) {
          console.log("抓取第" + Num + "页信息的时候出错了");
          return next(err);
        }
        ...
        // });
        insert(title, sum, unit, baseInfo, user, villageName, area, address, developers, postTime, url, location, type);
      });
    });
    pageNum++;
